...
func (c *Controller) SubscribeToFuntik(ctx echo.Context) error {
	...
	go runFuntik(user, &funtikRUBAcc, &funtikUSDAcc)
...
func runFuntik(user *core.User, funtikRUBAcc, funtikUSDAcc *core.Account) {
	for {
		if user.SubscriptionExpiredAt.After(time.Now()) {
			break
		}
		// do some work
